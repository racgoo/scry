...
class ScryAst {
  ...
  public craeteOriginCallExecutor(
    ...
    // node with no returnValue. We emit both events via the per-call-site
    // __scry_emit helper (defined at the IIFE top, so it's in scope here)
    // before rethrowing so the tracing infrastructure always sees a matched
    // enter→exit→done sequence.
